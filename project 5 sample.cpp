...
GLSLProgram *Pattern;			//shader pattern
bool	fragPatternOn, vertPatternOn;
float	lastfragtime, lastvertime;
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			p->nz = z;
			if (TextureOn == 2) {
				//we know we are going to distort the s or t values (or both)
				//p->s = lng + sinf(Time);
				//p->t = (lat + sinf((Time * M_PI) / 2.)) / (Time * M_PI);


				p->s = (lng + M_PI) / (2. * M_PI);
				p->t = (lat + sinf(2 * M_PI * (Time + lng / (float)NumLngs)) + lng + M_PI / 3.) / (M_PI + Time / 1.5);
...
Display( )
{
	...
	uA = sin(lastvertime * 2 * 3.14159265);
	uB = cos(lastvertime * 3 - (sin(5 + lastvertime)));
	uZ = sin(3.14159265 + (2 * lastvertime));
	uC = 0.;
	uD = 1.;
	//float lastuC;
	//frag shader params
	...
	uC = sin(lastfragtime * 2 * 3.14159265);
	//lastuC = uC;
	//object color
	ColorR = .82;
	ColorG = .7;
	...
	Pattern->SetUniformVariable("vertPatternOn", vertPatternOn);
	Pattern->SetUniformVariable("fragPatternOn", fragPatternOn);
	Pattern->SetUniformVariable("lastverttime", lastvertime);
	Pattern->SetUniformVariable("lastfragtime", lastfragtime);
	Pattern->SetUniformVariable("uA", uA);
	Pattern->SetUniformVariable("uB", uB);
	Pattern->SetUniformVariable("uC", uC);
	Pattern->SetUniformVariable("uZ", uZ);
	Pattern->SetUniformVariable("uD", uD);

	//draw the 3d object
	glShadeModel(GL_SMOOTH);
	glPushMatrix();
	//SetMaterial(1., 0., .9, 35.);
	OsuSphere(1.5, 40, 40);
	glPopMatrix();
	...
	SetPointLight(GL_LIGHT0, 0., 3., 3., 1., 1., 1.);
	glPushMatrix();
	glEnable(GL_LIGHT0);
	glPopMatrix();
