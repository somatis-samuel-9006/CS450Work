...
Display( )
{
	...
	//object 1, is shiny, is smooth, is lit, colored spotlight
	glPushMatrix();
	glTranslatef(-2., 0., 0.);
	...
	//do light 1
	SetSpotLight(GL_LIGHT0, -2., 1., 2.5, 0., -1., -1., 5., 5., 5.);
	glPushMatrix();
	if (light0on)
		glEnable(GL_LIGHT0);
	else
	...
	//object 2, shiny, smooth, lit texured
	glPushMatrix();
	glTranslatef(2., 0., 0.);
	//glShadeModel(GL_SMOOTH);
	SetMaterial(3., 3., 3., 17.);
	...
	//do light 2, white light
	SetSpotLight(GL_LIGHT1, 2., 1., 2.5, 0., -1., -1., 0., 9., 0.);
	glPushMatrix();
	if (light1on)
		glEnable(GL_LIGHT1);
	else
	...
	glDisable(GL_LIGHTING);
	glCallList(light1blob);
	glEnable(GL_LIGHTING);
	glPopMatrix();

	//do object 3, will be dull, flat, moving
	glPushMatrix();
	//try to animate
	//glRotatef(360. * Time, 1., 0., 0.);
	glTranslatef(4. * Time, 2., -2.);
	glShadeModel(GL_FLAT);
	SetMaterial(.9, .9, .9, 3.);
	glutSolidCube(1.0);
	glPopMatrix();

	//do light 3, moving point light
	SetSpotLight(GL_LIGHT2, 4. * Time, 2., -2., 0., -1., 1., 1., 0., 0.);
	...
	glDisable(GL_LIGHTING);
	glTranslatef(4. * Time, 0., 0.);
	glCallList(light2blob);
	glEnable(GL_LIGHTING);
	glPopMatrix();

	//do light 4
	SetPointLight(GL_LIGHT3, 0., -0.5, -0.5, 1., 1., 0.);
	glPushMatrix();
	if (light3on)
		glEnable(GL_LIGHT3);
	else
